...
export class ShareTrimPanelProvider implements vscode.WebviewViewProvider {
  ...
  resolveWebviewView(view: vscode.WebviewView): void {
    ...
    view.webview.onDidReceiveMessage((message: { type?: string; value?: boolean | number | string; id?: string }) => {
  ...
  private async onMessage(message: { type?: string; value?: boolean | number | string; id?: string }): Promise<void> {
    ...
    if (message.type === "deleteCache" && message.id) {
      await deleteCache(message.id);
      await this.postState();
      return;
    }
    if (message.type === "exportCache" && message.id) {
      await exportCache(message.id);
      return;
    }
    if (message.type === "forkCache" && message.id) {
      await forkCache(message.id);
      return;
    }
  ...
  <script nonce="${nonce}">
    ...
    const copy = {
      zh: {
        ...
        cacheTitle: "本地会话",
        cacheHint: "缓存在当前工作区的 .cursor/share-trim-cache。远程工作区时，列表在服务器上。导入从本机选 JSON，不打开服务器文件。Fork 会在本地新建会话，图片一起带回来。",
        refresh: "刷新",
        import: "从本机导入",
        del: "删除",
        export: "导出",
      ...
      en: {
        ...
        cacheHint: "Stored in the open workspace at .cursor/share-trim-cache. On a remote workspace, the list lives on the server. Import always picks a JSON file on this computer, not on the server. Fork creates a local chat and brings the images back.",
        refresh: "Refresh",
